...
# ============================================================================
# TRANSCRIPT NICHE SUMMARY — VIBE PROMPT ENRICHMENT
# ============================================================================

class TestVibePromptNicheSummary:
    """
    Verify that the Gemini vibe prompt includes transcript niche fields when
    transcript_niche_summary is populated, and falls back gracefully when empty.

    SCORING_VERSION compatibility guarantee: an empty transcript_niche_summary
    produces the same prompt structure as v2.0.x (no niche section injected).
    """

    def _make_seed_with_niche(self, niche_summary: dict) -> dict:
        return {
            'channel_id': 'UC_seed',
            'channel_name': 'Seed Channel',
            'subscriber_count': 100_000,
            'common_tags': ['python', 'programming'],
            'primary_keywords': ['python tutorials'],
            'secondary_keywords': ['coding'],
            'avg_engagement_rate': 0.05,
            'upload_frequency': 4.0,
            'recent_titles': ['Python Basics', 'Advanced Python'],
            'description_summary': 'A programming channel',
            'transcript_niche_summary': niche_summary,
        }
    ...
    def _make_candidate(self) -> dict:
        return {
            ...
            'recent_titles': ['Python OOP'],
        }

    def _capture_prompt(self, seed_profile: dict, candidate: dict) -> str:
        """Run gemini_similarity_analysis with a mock and return the captured prompt."""
        import re
        captured = []
        ...
        class _CapturingModel:
            def generate_content(self, prompt, **kwargs):
                ...
                resp = MagicMock()
                resp.text = "Score: 7/10\nReason: Similar Python focus"
                return resp

        callbacks = SimilarityCallbacks(debug_mode=True, on_api_call=Mock())
        with patch("app.core.similarity.genai") as mock_genai:
            mock_genai.configure = Mock()
            mock_genai.GenerativeModel.return_value = _CapturingModel()
            gemini_similarity_analysis(
    ...
    def test_niche_section_included_when_populated(self):
        niche = {
            'niche': 'Beginner Python programming tutorials',
        ...
        seed = self._make_seed_with_niche(niche)
        prompt = self._capture_prompt(seed, self._make_candidate())
        assert 'DEEP NICHE PROFILE' in prompt
        assert 'Beginner Python programming tutorials' in prompt
        assert 'new developers' in prompt
        assert 'tutorial' in prompt
        assert 'python basics' in prompt

    def test_niche_section_absent_when_empty_dict(self):
        seed = self._make_seed_with_niche({})
        prompt = self._capture_prompt(seed, self._make_candidate())
        assert 'DEEP NICHE PROFILE' not in prompt

    def test_niche_section_absent_when_key_missing(self):
        """Backward-compat: seed profiles without transcript_niche_summary at all."""
        seed = self._make_seed_with_niche({})
        del seed['transcript_niche_summary']
        prompt = self._capture_prompt(seed, self._make_candidate())
        assert 'DEEP NICHE PROFILE' not in prompt

    def test_confidence_shown_in_prompt(self):
        niche = {
